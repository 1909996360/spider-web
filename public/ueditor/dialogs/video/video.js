...
OnlineImage.prototype = {
    ...
    pushData: function (list) {
        ...
        for (i = 0; i < list.length; i++) {
            if(list[i] && list[i].filePath) {
                ...
                img.width = 113;
                img.setAttribute('src', urlPrefix + list[i].thumbnail + (list[i].filePath.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
        ...
        //    console.log(img);
            if(img && img.className){
                ...
                if(hasFakedClass || hasUploadClass) {
                    $G("videoUrl").value = url = img.getAttribute("source-src");
                    poster=img.getAttribute("src");
                    // $G("videoWidth").value = img.width;
                    // $G("videoHeight").value = img.height;
                   // var align ="center";// domUtils.getComputedStyle(img,"float"),
    ...
    function insertSingle(){
        var width = "100%",
            ...
            poster = $($("#preview").find("video")[0]).attr("_poster"),
            align = "center";//findFocus("videoFloat","name");
          ///  console.log('==========================');
           // console.log(poster);
            poster = poster==null||poster==""?"/ueditor/dialogs/video/images/play.png":poster;
        if(!url) return false;
    ...
    function createPreviewVideo(url,poster){
       ...
       // console.log(conUrl);
        ...
        '<video class="edui-upload-video  vjs-default-skin video-js"  controls=""  width="100%" height="100%" src="' +conUrl + '" data-setup="{}" _poster="'+poster+'"> <source src=" ' +conUrl + '" type="video/mp4"/> </video>';
    }
