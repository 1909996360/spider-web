...
OnlineImage.prototype = {
    ...
    getImageData: function () {
        ...
        if(!_this.listEnd && !this.isLoadingData) {
            ...
            var url = window.UE.getSuiyueHost()+"/api/pc/fileUpload/selectFileUploadPage";
            $.ajax({
                url:url,
                type: 'GET',
                data:{
                ...
                success:function (json) {
                        if(json.code == '200'&&json.dataList){
                            _this.pushData(json.dataList);
                           // _this.listIndex = json.maxCount + (json.currentPage-1)*50;
                           // if(_this.listIndex >= json.maxPage*50) {
                                _this.listEnd = true;
                           // }
    ...
    pushData: function (list) {
        ...
        for (i = 0; i < list.length; i++) {
            if(list[i] && list[i].filePath) {
                ...
                img.style = "width:380px;height :94px;";
                img.setAttribute('src', urlPrefix + (list[i].thumbnail==null?editor.options.langPath + editor.options.lang+"/images/playbg.png":list[i].thumbnail) + (list[i].filePath.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                img.setAttribute('_src', urlPrefix + list[i].filePath);
                img.setAttribute('_name',  list[i].fileName);
    ...
    getInsertList: function () {
        var list=[];
        var lis =  $("#imageList").find("li");
        for (var i = 0; i < lis.length; i++) {
            if (domUtils.hasClass(lis[i], 'selected')) {
                var img = lis[i].firstChild;
                
                var url = img.getAttribute('_src');
                var name = img.getAttribute('_name');
           ...
           // console.log(img);
            if(img && img.className){
                ...
                if(hasFakedClass) {
                    ...
                    $G("videoName").value = img.getAttribute("_name");
        ...
        dialog.onok = function(){
            ...
            switch(currentTab){
                ...
                case "videoSearch":
                    ...
                    break;
                case "online":  
                var list=onlineImage.getInsertList();
    ...
    function insertSingle(){
        var width = "100%",
            ...
            name = $G('videoName').value,
            img=$($("#preview").find("audio")[0]).attr("_img"),
            align = "center";
          //  console.log("!!!!!!!!!!!!!"+img);
        if(!url) return false;
    ...
    function insertSearch(id){
        ...
        editor.execCommand('music',videoObjs);
    }
    ...
    function createPreviewVideo(url,poster){
       ...
       // var conUrl = convert_url(url);
        ...
        $G("preview").innerHTML = '<audio class="edui-faked-music" controls="" style="width:100%;"  width="100%" height="100%" src="' + url + '" _img="'+poster+
                                    '"> <source src=" ' + url + '" type="audio/*"/> </audio>';
     }
